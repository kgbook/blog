+++
title = "给不能上外网的编译服务器配置 Codex：让 AI 写代码、修 bug、自动修编译错误"
date = 2026-06-17
...
这很常见：服务器在机房、实验室、内网环境里，能访问代码仓库和构建依赖，但不能直接访问 OpenAI、GitHub、npm、pip 等外部服务。以前这只是“下载依赖麻烦一点”，现在多了一个新需求：我们想在这台服务器上跑 Codex，让 AI 直接读代码、改代码、看编译错误，然后继续修，确保编译通过。
...
本文实际使用的端口是：`7897`，有些配置会分成 HTTP、SOCKS、Mixed Port。新手建议优先用 Mixed Port。
...
DHCP 是动态分配地址的协议。普通情况下，电脑每次联网时向路由器申请一个 IP，路由器可能今天分配 `192.168.1.100`，下次分配另一个地址。

但是很多路由器支持“DHCP 地址保留”或“静态租约”：路由器仍然通过 DHCP 发地址，但它会根据网卡 MAC 地址，把同一块网卡永远分配到同一个 IP。这个做法也常被路由器界面叫做 **IP 与 MAC 绑定**。
...
![路由器 LAN 配置](images/router-LAN-configuration.png)
...
跨网段场景下，编译服务器应该访问 `10.48.10.14:7897`。如果这个端口不通，就查防火墙、路由器转发、Clash verge等配置。
